...
function cast.check_song(song_list,targ,buffs,spell_recasts,recasts,JA_WS_lock,recast)
    ...
    for i = 1, math.min(#song_list, maxsongs) do
        ...
        if song and spell_recasts[song.id] <= 0 and
			...
			if ta == '<me>' and settings.clarion and not JA_WS_lock and not buffs.clarion and recasts[254] <= 0 and recasts[109] <= 0 and recasts[110] <= 0 then
                cast.JA('input /ja "Clarion Call" <me>')
			elseif ta == '<me>' and settings.soul_voice and not JA_WS_lock and not buffs.soul_voice and recasts[0] <= 0 and recasts[109] <= 0 and recasts[110] <= 0 then
